...
where
    S: Subscriber + for<'a> LookupSpan<'a>,
{
    let file_layer = config.write_to_files.map(make_file_layer);

    let (reload_layer, handle) = reload::Layer::new(file_layer);
    RELOAD_HANDLE.set(handle).expect("Failed to set reload handle");
...
fn make_file_layer(
    file_config: TracingFileConfiguration,
) -> impl Layer<tracing_subscriber::registry::Registry> {
    ...
    if let Some(file_suffix) = file_config.file_suffix {
        builder = builder.filename_suffix(file_suffix)
    }

    let writer =
        builder.build(&file_config.path).expect("Failed to create a rolling file appender.");
...
static RELOAD_HANDLE: OnceLock<Handle<Option<dyn Layer<Registry>>, Registry>> = OnceLock::new();
...
#[matrix_sdk_ffi_macros::export]
pub fn reload_file_config(file_config: TracingFileConfiguration) -> Result<(), ClientError> {
    if let Some(handle) = RELOAD_HANDLE.get() {
        let layer = make_file_layer(file_config);
        handle.reload(layer).map_err(|e| ClientError::Generic {
            msg: "Failed to reload file config".to_owned(),
